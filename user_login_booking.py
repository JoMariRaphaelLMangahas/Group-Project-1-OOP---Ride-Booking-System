...
class MobileApp:
    def __init__(self, root):
        ...
        self.header_frame = tk.Frame(root, bg="#F6D2E0", height=60) 
        ...
        self.exit_button = tk.Button(self.header_frame, text="X", font=("Helvetica", 13), bg="#C8E7F5", width=4, highlightthickness=3, command=self.root.destroy)
        self.exit_button.place(relx=0.990, rely=0.24, anchor='ne')

        self.content_frame = tk.Frame(root, bg="#C8E7F5")
        ...
        self.footer_frame = tk.Frame(root, bg="#F6D2E0", height=50)
    ...
    def login(self):
        # Handle login (for now just open main app window)
        self.open_main_app()

    def open_main_app(self):
        self.root.destroy()
        main_app = tk.Tk()
        ride_app = main.RideApp(main_app)
        main_app.mainloop()
